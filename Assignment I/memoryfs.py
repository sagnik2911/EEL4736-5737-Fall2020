...
## Prints out file system information
  ...
  def PrintFSInfo(self):
    ...
    logging.info('File Entries per datablock (Name and Inode)  : ' + str(FILE_ENTRIES_PER_DATA_BLOCK))
...
## returns number of bytes written
  ...
  ## Reads data to a file, starting at offset
  ## offset must be less than or equal to the file's size
  ## count is number of blocks to read
  ## returns the read blocks

  def Read(self, file_inode_number, offset, count):

    data = bytearray()
    logging.debug ("Read: file_inode_number: " + str(file_inode_number) + ", offset: " + str(offset) + ", len(data): " + str(count))
    ...
    while bytes_read < count:
      ...
      if (offset + count) >= next_block_boundary:
        ...
        # otherwise, the data is truncated within this block
        read_end = (offset + count) % BLOCK_SIZE
      ...
      # copy slice of block into the right position of data
      data[bytes_read:bytes_read + (read_end - read_start)] = block[read_start:read_end]
      logging.debug('Blocks Read : ' + str(data.decode()))
